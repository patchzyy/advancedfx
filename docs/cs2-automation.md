...
- `artifactAgrModelChecks`: per-artifact regex checks against parsed AGR model arrays such as `models`, `viewModelModels`, and `worldWeaponModels`.
...
- `tools/cs2-automation/scenarios/shinden-agr-entity-parity.json` is the section 3 entity-data regression. On the verified run from March 9, 2026, it passed and proved player exports now exclude `C_CSObserverPawn`, player records still carry bones, visible-world weapons stay separate from hidden carried inventory, projectiles classify independently, `recordViewModels -1` still exports viewmodel pieces with `recordInvisible 0`, and `recordWeapons` no longer grows unique handle coverage when `recordInvisible 1` is enabled at the sampled tick window.
- `tools/cs2-automation/scenarios/shinden-agr-animation-fidelity.json` is the section 4 animation / transform regression. On the verified run from March 9, 2026, it passed and proved all three spectator cases (`spec_mode 4` POV, `spec_mode 5` observer-target chase, and `spec_mode 6` HLTV/freecam) still produce valid AGR output, that POV / observer captures keep non-trivial viewmodel skeleton export (`maxBoneCount > 1`), that parsed recordings show moving entity transforms instead of only static frame snapshots, and that observer pawn models no longer leak into player exports.
- `tools/cs2-automation/scenarios/shinden-agr-toggle-matrix.json` is the broad section 5 toggle regression. On the verified run from March 9, 2026, it passed and proved player-only exports now contain `10` unique player handles instead of `10` players plus `10` observer pawns, `recordPlayerCameras -1` emits one camera record per exported player state, and `recordWeapons 1` with `recordInvisible 1` matches the same `2` unique visible-world weapon handles as `recordWeapons 1` at the sampled ticks instead of flooding the AGR with hidden inventory.
- `tools/cs2-automation/scenarios/shinden-agr-ragdoll-verification.json` is the section 5 ragdoll regression. On the verified run from March 9, 2026, it passed at the confirmed Dust2 death window (`demo_gototick 11000`, `spec_player 6`, `spec_mode 4`, `demo_timescale 0.25`) and proved CS2 AGR now records dead `C_CSPlayerPawn` states as ragdoll exports with bones instead of missing them entirely.
- `tools/cs2-automation/scenarios/shinden-agr-problem-ticks.json` is the section 6 problematic-tick regression. On the verified run from March 9, 2026, it passed and proved POV imports still contain moving weapon viewmodels with bones, `spec_mode 5` still keeps the spectated weapon path alive, hidden entries are emitted after a mid-recording toggle change, and observer pawn models do not leak into imported AGR player/model sets.
- `tools/cs2-automation/scenarios/shinden-agr-player-camera-fov.json` is the section 6 player-camera FOV regression. On the verified run from March 9, 2026, it passed and proved a dedicated per-player camera export still writes `camera` payloads and that the first player-camera FOV matches the first main-camera FOV within a `0.05` tolerance.
...
- The AGR parser now also exposes import-facing model sets (`models`, `viewModelModels`, `worldWeaponModels`), unique world/viewmodel handle counts, moving viewmodel handle counts, and first/last player-camera FOV values. Use those parsed properties for importer-style validation instead of raw string scraping or cross-file size guesses.
