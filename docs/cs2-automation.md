...
- `artifactAgrChecks`: per-artifact AGR payload checks against parsed counters such as `entityCount`, `mainCameraCount`, `playerCameraCount`, `viewModelEntityCount`, `invisibleEntityCount`, `boneEntityCount`, and `boneCount`.
...
- The AGR parser also exposes unique-handle counts plus first / last main-camera samples, so scenarios can validate stable handle reuse and exact `afxCam` payload values instead of only checking for file existence.
